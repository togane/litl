			...
			if ( isset( $option['type'] ) ) {
				...
				switch ( $option['type'] ) {
					...
					case 'ckecksorter':
						...
						$wp_customize->add_control(
							new Customizer_Library_Check_Sortable(
								$wp_customize, $option['id'], $option
							)
						);
					...
					case 'category':
						...
						$wp_customize->add_control(
							new Customizer_Library_Category(
								$wp_customize, $option['id'], $option
							)
						);
...
function customizer_library_get_sanitization( $type ) {
	...
	if ( 'color' == $type ) {
		return 'customizer_library_sanitize_hex_color';
	}
	...
	if ( 'textarea' == $type ) {
		return 'customizer_library_sanitize_textarea';
	}

	if ( 'ckecksorter' == $type ) {
		return 'customizer_library_multicheck_sortable_sanitaze';
	}
	...
	if ( 'dropdown-pages' == $type ||  'category' == $type ) {
		return 'absint';
	}
