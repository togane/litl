 ...
 * PicSellろぐ
 */
function add_post_type_picsell()
{
        $args = array(
                'label'           => 'PicSell',
                'public'          => true,
      ...
      'page-attributes',
        ...
        register_post_type('picsell', $args);
        register_taxonomy(
                'picsellcat',
                'picsell',
                array(
...
add_action('init', 'add_post_type_picsell');

// PicSell一覧ページの表示件数
add_action('pre_get_posts', 'my_pre_get_posts'); 
function my_pre_get_posts($query) {
if (!is_admin() && $query->is_main_query() && is_post_type_archive('picsell')) {
  $query->set('posts_per_page', 6);
  $query->set('order', 'ASC'); // 昇順
  $query->set('orderby', 'date' );
  }
}
// PicSell投稿ページのリビジョン表示
function add_posttype_revisions() {
    add_post_type_support( 'picsell', 'revisions' );
}
add_action('init', 'add_posttype_revisions');


// 管理画面の位置にパーマリンクを表示
function add_custom_picsell( $defaults ) {
  $defaults['permalink'] = 'パーマリンク';
  return $defaults;
}
add_filter('manage_posts_picsells', 'add_custom_picsell');

function add_custom_picsell_id($picsell_name, $id) {
  if($picsell_name == 'permalink'){
    echo get_permalink();
  }
}
add_action('manage_posts_custom_picsell', 'add_custom_picsell_id', 10, 2);
