...
if ( ! class_exists( 'Shapla_Breadcrumb' ) ) {
	/**
	...
	class Shapla_Breadcrumb {
		...
		public function get_breadcrumb() {
			return apply_filters( 'shapla_get_breadcrumb', $this->crumbs, $this );
		}
		...
		private function add_crumbs_404() {
			$this->add_crumb( __( 'Error 404', 'shapla' ) );
		}
		...
		private function add_crumbs_author() {
			...
			$this->add_crumb( sprintf( __( 'Author: %s', 'shapla' ), $userdata->display_name ) );
		}
		...
		private function paged_trail() {
			if ( get_query_var( 'paged' ) ) {
				$this->add_crumb( sprintf( __( 'Page %d', 'shapla' ), get_query_var( 'paged' ) ) );
			}
		}
	}
}
